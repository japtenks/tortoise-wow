...
realm_line="$(find_realm_line ptr)" || {
  echo "PTR realm not found in REALM_MATRIX." >&2
  exit 1
}
IFS='|' read -r realm_slug world_ctid realm_id realm_name realm_address world_port <<< "${realm_line}"

echo "==> Updating PTR realm from branch '${PTR_REPO_BRANCH}'"
env \
